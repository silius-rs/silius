...
pub trait AddRemoveUserOp {
    ...
    ///
    /// # Returns
    /// * `Ok(bool)` - true if the [UserOperation](UserOperation) was removed, false means it was not found
...
pub trait UserOperationOp {
    ...
    /// # Returns
    ///
    /// Returns `Ok(Vec<UserOperation>)` containing all user operations sorted in the specified order,
...
pub trait UserOperationCodeHashOp {
    ...
    /// - If an error occurs during the operation, an `Err` variant is returned with a `MempoolError`.
    ...
    /// Retrieves the code hashes associated with the given user operation hash from the memory pool.
    ///
    /// # Arguments
    ///
    ...
    ///
    /// - If the code hashes are successfully retrieved, `Ok(Vec<CodeHash>)` is returned with the code hashes.
    /// - If an error occurs during the operation, an `Err` variant is returned with a `MempoolError`.
