...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let silius_version = git_describe.split('-').collect::<Vec<&str>>()[0];
    println!("cargo:rustc-env=SILIUS_VERSION={silius_version}");
    ...
    println!("cargo:rustc-env=SILIUS_BUILD_OPERATING_SYSTEM={build_operating_system}");

    // The architecture of the build, x86_64, aarch64, etc.
    println!("cargo:rustc-env=SILIUS_BUILD_ARCHITECTURE={build_architecture}");

    println!(
        "cargo:rustc-env=SILIUS_FULL_VERSION={}",
        format_args!(
            "{version}-{hash} {build_os}-{build_arch} rustc{rust_version}",
            version = silius_version,
            hash = short_sha,
