use crate::{
    contracts::{
        gen::entry_point_api::{self, ValidationResult},
        tracer::JsTracerFrame,
        EntryPoint, EntryPointErr, SimulateValidationResult,
    },
...
const REVERT_OPCODE: &str = "REVERT";

lazy_static! {
    static ref FORBIDDEN_OPCODES: HashSet<&'static str> = HashSet::from([
        "GASPRICE",
        "GASLIMIT",
        "DIFFICULTY",
        "TIMESTAMP",
...
impl<M: Middleware + 'static> UoPoolService<M> {
    ...
    async fn ended_with_revert(
        &self,
        frame: &JsTracerFrame,
    ) -> Result<(), UserOperationSimulationError> {
        if let Some(last_call) = frame.calls.last() {
            if last_call.typ == REVERT_OPCODE {
                return Ok(());
            }
        }

        Err(UserOperationSimulationError::Simulation(
            BadUserOperationError::NotReverted,
        ))
    }

    async fn check_forbidden_opcodes(
        &self,
        frame: JsTracerFrame,
    ) -> Result<(), UserOperationSimulationError> {
        Ok(())
    ...
    async fn simulate_user_operation(
        ...
        //     .await?;
        let simulation_result = ValidationResult {
            return_info: (
                U256::from(388901),
                U256::from(881985594867704_u64),
                false,
                0,
                0,
                Bytes::default(),
